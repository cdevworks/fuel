...
import "../token/RUPEETokenInterface.sol";


contract RUPEERebaser {
     ...
     * @notice Sets the reserve contract
     */
    event TreasuryIncreased(uint256 reservesAdded, uint256 rupeesSold, uint256 rupeesFromReserves, uint256 rupeesToReserves);
    ...
    /// @notice RUPEE token address
    address public rupeeAddress;
    ...
    /// @notice pair for reserveToken <> RUPEE
    ...
    /// @notice Whether or not this token is first in uniswap RUPEE<>Reserve pair
    bool public isToken0;

    constructor(
        address rupeeAddress_,
        ...
        public
    {
          ...
          (address token0, address token1) = sortTokens(rupeeAddress_, reserveToken_);

          // used for interacting with uniswap
          if (token0 == rupeeAddress_) {
              isToken0 = true;
          } else {
              isToken0 = false;
          }
          // uniswap RUPEE<>Reserve pair
    ...
    */
        ...
        public
    {
        ...
        RUPEETokenInterface rupee = RUPEETokenInterface(rupeeAddress);

        if (positive) {
            require(rupee.rupeesScalingFactor().mul(uint256(10**18).add(indexDelta)).div(10**18) < rupee.maxScalingFactor(), "new scaling factor will be too big");
        }


        uint256 currSupply = rupee.totalSupply();
        ...
        // rebase
        uint256 supplyAfterRebase = rupee.rebase(epoch, indexDelta, positive);
        assert(rupee.rupeesScalingFactor() <= rupee.maxScalingFactor());
        ...
        public
    {
        ...
        if (uniVars.amountFromReserves > 0) {
            ...
            rupee.transferFrom(reservesContract, uniswap_pair, uniVars.amountFromReserves);
            if (uniVars.amountFromReserves < uniVars.rupeesToUni) {
                // if the amount from reserves > rupeesToUni, we have fully paid for the yCRV tokens
                // thus this number would be 0 so no need to mint
                rupee.mint(uniswap_pair, uniVars.rupeesToUni.sub(uniVars.amountFromReserves));
            }
        } else {
            // mint to uniswap
            rupee.mint(uniswap_pair, uniVars.rupeesToUni);
        }

        // mint unsold to mintAmount
        if (uniVars.mintToReserves > 0) {
            rupee.mint(reservesContract, uniVars.mintToReserves);
        }

        // transfer reserve token to reserves
        if (isToken0) {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount1);
            emit TreasuryIncreased(amount1, uniVars.rupeesToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        } else {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount0);
            emit TreasuryIncreased(amount0, uniVars.rupeesToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        }
    }
        ...
        internal
    {
        ...
        // check if protocol has excess rupee in the reserve
        uint256 excess = rupee.balanceOf(reservesContract);


        uint256 tokens_to_max_slippage = uniswapMaxSlippage(token0Reserves, token1Reserves, offPegPerc);

        UniVars memory uniVars = UniVars({
          rupeesToUni: tokens_to_max_slippage, // how many rupees uniswap needs
          amountFromReserves: excess, // how much of rupeesToUni comes from reserves
          mintToReserves: 0 // how much rupees protocol mints to reserves
        });
        ...
        if (isToken0) {
            if (tokens_to_max_slippage > mintAmount.add(excess)) {
                ...
                uniVars.rupeesToUni = mintAmount + excess;
                ...
                if (tokens_to_max_slippage > excess) {
                    ...
                    // swap up to slippage limit, taking entire rupee reserves, and minting part of total
